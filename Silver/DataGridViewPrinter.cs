...
namespace Silver
{
    class DataGridViewPrinter
    {
        ...
        private bool IsSelection; // Determine if the report will contain only selected rows
        ...
        public DataGridViewPrinter(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage,
            bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging, bool Selection)
        {
        ...
        private void Calculate(Graphics g)
        {
            if (PageNumber == 0) // Just calculate once
            {
                ...
                for (int i = 0; i < TheDataGridView.Columns.Count; i++)
                {
                    ...
                    {
         //               if (IsSelection && !TheDataGridView.Rows[j].Selected) continue;
        ...
        private void DrawHeader(Graphics g)
        {
            ...
            {
                PageNumber++;
                string PageString = "Страница " + PageNumber.ToString();
        ...
        private bool DrawRows(Graphics g)
        {
            ...
            {
                if ((!IsSelection && TheDataGridView.Rows[CurrentRow].Visible)
                    || (IsSelection && TheDataGridView.Rows[CurrentRow].Selected)) // Print the cells of the CurrentRow only if that row is visible
                {
